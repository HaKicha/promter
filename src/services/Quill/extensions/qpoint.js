import Quill from 'quill';

const Embed = Quill.import('blots/block/embed');
const img = `data:image/svg+xml;base64,`;

class Hr extends Embed {
    static create(value) {
        let node = super.create(value);
        let times = document.createElement('img');
        times.setAttribute('src', img);
        times.setAttribute('height', '16');
        times.setAttribute('width', '16');
        times.setAttribute('style', 'cursor: pointer;');
        node.innerHTML = '<div style="width: 100%; border-bottom: 1px solid rgba(0,0,0,0.5)"/>'
        node.setAttribute(
            'style',
            "display:grid; grid-template-columns: 1fr 24px; grid-gap: 20px; align-items: center; margin: 10px 0; opacity: 0.5;"
        );

        const listener = () => {
            node.removeEventListener('click',listener)
            node.remove();
        };
        times.addEventListener('click', listener)
        node.appendChild(times)
        return node;
    }
}

Hr.blotName = 'hr';
Hr.className = 'que-point';
Hr.tagName = 'div';

Quill.register({
    'formats/hr': Hr
});